// Unified API client for campers-api.goit.study
...
export async function getCampers(params?: QueryParams): Promise<Camper[]> {
  return apiFetch<Camper[]>("/campers", { params });
}

export async function getCamper(id: string): Promise<Camper> {
  return apiFetch<Camper>(`/campers/${id}`);
}
